...
| --- | --- |
| users | 用户账号 |
| accounts | 第三方账号预留，可先不用 |
| sessions | 登录会话 |
| verification_tokens | 找回密码、邮箱验证 |
...
推荐 Auth.js/NextAuth + PostgreSQL adapter，或者自己写极简 email/password 登录。更省返工的是 Auth.js，因为会话、token、回调这些坑它已经踩过。
...
3. Auth 方案：Auth.js 优先，除非你明确想手写账号密码。
4. 文件存储边界：本地目录起步，生产再换 S3/MinIO。
5. AI 任务边界：第一版同步请求，队列以后再加。
6. 环境变量：`DATABASE_URL`、auth secret、上传目录、后续 `LITELLM_PROXY_URL` 和 `AI_CONFIG_ENCRYPTION_KEY`。
...
3. Auth 是否用 Auth.js/NextAuth，避免手写会话？
